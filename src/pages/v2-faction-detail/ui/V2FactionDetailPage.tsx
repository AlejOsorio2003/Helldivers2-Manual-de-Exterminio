  ...
  return (
    <div className="shrink-0 w-full" style={{ borderTop: `1px solid ${color}20` }}>
      <div className="flex items-center justify-center gap-1.5 py-3">
        ...
        {slots.map((slot, i) => {
          if (slot === null) return <span key={`e-${i}`} className="w-7 h-7 shrink-0" />
          if (slot === '…') return (
            <span key={`ell-${i}`} className="w-7 h-7 shrink-0 flex items-center justify-center font-mono text-xs text-text-muted/40">…</span>
          )
